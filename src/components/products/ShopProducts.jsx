...
const ShopProducts = ({ styles, products }) => {
    ...
    products.map((p, i) => (
        <div
            key={i}
            className={`flex transition-all duration-1000 hover:shadow-md hover:-translate-y-3 ${styles === 'grid' ? 'flex-col justify-start items-start' : 'justify-start items-center md-lg:flex-col md-lg:justify-start md-lg:items-start'} w-full gap-4 bg-white p-1 rounded-md border border-slate-400`}
    ...
    return (
        <div className={`w-full grid ${styles === 'grid' ? 'grid-cols-4 md-lg:grid-cols-2 md:grid-cols-3 sm:grid-cols-2' : 'grid-cols-1 md-lg:grid-cols-4 md:grid-cols-4'} gap-3`}>
            {
                products.map((p, i) => <div key={i} className={`flex transition-all duration-1000 hover:shadow-md hover:-translate-y-3 ${styles === 'grid' ? 'flex-col justify-start items-start' : 'justify-end items-center md-lg:flex-col md-lg:justify-end md-lg:items-end'} w-full gap-4 bg-white border border-slate-200 rounded-md`}>
                    <div className={styles === 'grid' ? 'w-full relative group md:h-[270px] xs:h-[170px] overflow-hidden' : 'md-lg:w-full relative group h-[210px] md:h-[270px] overflow-hidden'}>
                        <img className='h-[240px] md:h-[270px] xs:h-[170px] w-full object-cover' src={p.images[0]} alt="image" />
                        <ul className='flex transition-all duration-700 -bottom-10 justify-center items-center gap-2 absolute w-full group-hover:bottom-3'>
                    ...
                    <div className='py-3 text-slate-600 px-2'>
                            ...
                            {
                                p.discount ? <div className='flex justify-center items-center line-through text-red-400'> {p.price + p.discount} ETB</div> : ""
                            }
